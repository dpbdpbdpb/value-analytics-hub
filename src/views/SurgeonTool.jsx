        ...
        {activeView === 'system' && (() => {
          ...
          return (
            <div className="space-y-3">
              ...
              {/* Six Consolidation Scenarios */}
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Seven Paths Forward: Vendor Consolidation Scenarios</h3>
                <p className="text-gray-600 mb-3">
                  Each scenario represents a different combination of approved vendors. <strong>Click a scenario to explore its details below.</strong>
                </p>

                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2">
                  {Object.entries(scenarios).map(([key, scenario]) => {
                    const projection = systemContext.scenarioProjections[key];
                    const isProjAvail = !!projection;
                    ...
                    return (
                      <button
                        ...
                        className={`text-left bg-gradient-to-br from-purple-50 to-blue-50 rounded-lg p-5 border-2 transition-all hover:shadow-md ${
                          isSelected ? 'border-purple-600 ring-2 ring-purple-300' : 'border-purple-200 hover:border-purple-400'
                        }`}
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className="text-2xl font-bold text-purple-600">Scenario {key}</div>
                          {isProjAvail && projection.potential > 0 && (
                            <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full font-bold">
                              Top {Math.ceil((projection.potential / systemContext.systemSavingsGoal) * 100)}%
                            </span>
                          )}
                        </div>
                        ...
                        <div className="space-y-2 text-sm mb-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-bold text-green-600">{(scenario.savings * 100).toFixed(0)}%</span>
                          </div>
                          {isProjAvail && (
                            <>
                              <div className="flex items-center justify-between">
                                <span className="text-gray-600">Potential:</span>
                                <span className="font-bold text-blue-600">${(projection.potential / 1000000).toFixed(1)}M</span>
                              </div>
                              <div className="flex items-center justify-between">
                                <span className="text-gray-600">Surgeons Affected:</span>
                                <span className="font-bold">{projection.surgeons} <span className="text-purple-600">({(projection.adoptionRate * 100).toFixed(0)}%)</span></span>
                              </div>
                            </>
                          )}
                        </div>
                        ...
                        <div className="border-t pt-3">
                          <div className="text-xs font-semibold text-gray-500 mb-2">Approved Vendors:</div>
                          <div className="flex flex-wrap gap-1">
                            {scenario.vendors.map(vendor => (
                              <span key={vendor} className="text-xs bg-white border border-purple-200 rounded px-2 py-1">
                                {vendor.split(' ')[0]}
                              </span>
                            ))}
                          </div>
              ...
              {/* Clinical Quality Protection and The Opportunity - Side by Side */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">

              {/* Clinical Quality Protection */}
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Clinical Quality Protection</h3>

                <div className="bg-green-50 border-l-4 border-green-500 p-3 mb-3">
                  <p className="text-xs text-gray-800 leading-relaxed">
                    <strong>All consolidation scenarios maintain equivalent clinical outcomes.</strong> Cost optimization does not compromise patient care.
                  </p>
                </div>

                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <h4 className="font-semibold text-gray-800 text-sm mb-2">10-Yr Survivorship</h4>
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">STRYKER</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.survivorship['STRYKER']}%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">ZIMMER</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.survivorship['ZIMMER BIOMET']}%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">J&J</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.survivorship['J&J']}%</span>
                      </div>
                      <div className="flex justify-between items-center border-t pt-1">
                        <span className="text-gray-600">Benchmark</span>
                        <span className="font-bold text-gray-600">{systemContext.clinicalData.survivorship.benchmark}%</span>
                      </div>
                    </div>
                  </div>

                  <div>
                    <h4 className="font-semibold text-gray-800 text-sm mb-2">30-Day Complicat.</h4>
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">STRYKER</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.complications['STRYKER']}%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">ZIMMER</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.complications['ZIMMER BIOMET']}%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">J&J</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.complications['J&J']}%</span>
                      </div>
                      <div className="flex justify-between items-center border-t pt-1">
                        <span className="text-gray-600">National</span>
                        <span className="font-bold text-gray-600">{systemContext.clinicalData.complications.national}%</span>
                      </div>
                    </div>
                  </div>

                  <div>
                    <h4 className="font-semibold text-gray-800 text-sm mb-2">5-Yr Revision</h4>
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">STRYKER</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.revisionRates['STRYKER']}%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">ZIMMER</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.revisionRates['ZIMMER BIOMET']}%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-700">J&J</span>
                        <span className="font-bold text-green-600">{systemContext.clinicalData.revisionRates['J&J']}%</span>
                      </div>
                      <div className="flex justify-between items-center border-t pt-1">
                        <span className="text-gray-600">National</span>
                        <span className="font-bold text-gray-600">{systemContext.clinicalData.revisionRates.national}%</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-2 bg-gray-50 rounded-lg p-2">
                  ...
                  <div className="text-xs text-gray-600">
                    AJRR 2023 • CMS 2020-2023 • CommonSpirit 2024
                  </div>
                </div>
              </div>

              {/* The Opportunity - Dynamic Based on Selected Scenario */}
              <div className="bg-white rounded-xl shadow-lg p-4">
                <h3 className="text-xl font-bold text-gray-900 mb-2">The Opportunity: Scenario {selectedScenario}</h3>
                <p className="text-xs text-gray-600 mb-3">{scenarios[selectedScenario].name}</p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4">
                    <h4 className="font-semibold text-gray-800 mb-2 text-center text-sm">Current State</h4>
                    <div className="text-center mb-2">
                      <div className="text-3xl font-bold text-red-600">12+</div>
                      <div className="text-xs text-gray-600 mt-1">Different vendors</div>
                    </div>
                    <p className="text-xs text-gray-700 text-center">
                      Fragmented = Weak negotiating power
                    </p>
                  </div>
                  <div className="bg-green-50 border-2 border-green-200 rounded-lg p-4">
                    <h4 className="font-semibold text-gray-800 mb-2 text-center text-sm">Target State</h4>
                    <div className="text-center mb-2">
                      <div className="text-3xl font-bold text-green-600">{scenarios[selectedScenario].vendors.length}</div>
                      <div className="text-xs text-gray-600 mt-1">Preferred vendor{scenarios[selectedScenario].vendors.length > 1 ? 's' : ''}</div>
                    </div>
                    <p className="text-xs text-gray-700 text-center">
                      {(scenarios[selectedScenario].savings * 100).toFixed(0)}% savings potential
                    </p>
                  </div>
                </div>

                <div className="mt-3 bg-purple-50 border-l-4 border-purple-500 p-3">
                  <div className="text-xs font-semibold text-purple-900 mb-1">Approved Vendors:</div>
                  <div className="flex flex-wrap gap-1">
                    {scenarios[selectedScenario].vendors.map(vendor => (
                      <span key={vendor} className="text-xs bg-white border border-purple-300 rounded px-2 py-1 font-semibold">
                        {vendor}
                      </span>
                    ))}
                  </div>
                ...
                {systemContext.scenarioProjections[selectedScenario] && (
                  <div className="mt-3 bg-blue-50 border-l-4 border-blue-500 p-3">
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <div className="text-gray-600">Annual Savings</div>
                        <div className="text-lg font-bold text-blue-600">
                          ${(systemContext.scenarioProjections[selectedScenario].potential / 1000000).toFixed(1)}M
                        </div>
                      </div>
                      <div>
                        <div className="text-gray-600">Surgeons Affected</div>
                        <div className="text-lg font-bold text-gray-900">
                          {systemContext.scenarioProjections[selectedScenario].surgeons} <span className="text-sm text-purple-600">({(systemContext.scenarioProjections[selectedScenario].adoptionRate * 100).toFixed(0)}%)</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            {(() => {
              ...
              const getVendorPricingForComponent = (category, surgeonVendor) => {
                ...
                surgeonData.forEach(surgeon => {
                  surgeon.topComponents.forEach(comp => {
                    if (comp.category === category && comp.avgPrice >= 100 && comp.avgPrice <= 10000) {
                      if (!vendorPricing[comp.vendor]) {
                        vendorPricing[comp.vendor] = { prices: [], totalQuantity: 0 };
                      }
                      vendorPricing[comp.vendor].prices.push(comp.avgPrice);
                      vendorPricing[comp.vendor].totalQuantity += comp.quantity;
                    }
                  });
                });

                // Calculate median price for each vendor
                const allowedVendors = ['ZIMMER BIOMET', 'STRYKER', 'J&J', 'SMITH & NEPHEW'];
                const vendorStats = Object.entries(vendorPricing)
                  .filter(([vendor]) => allowedVendors.includes(vendor))
                  .map(([vendor, data]) => {
                ...
                // Check if surgeon's vendor is in top 4, if not add it as 5th column
                const top4Vendors = vendorStats.slice(0, 4).map(v => v.vendor);
                if (surgeonVendor && !top4Vendors.includes(surgeonVendor) && vendorPricing[surgeonVendor]) {
                  const surgeonVendorData = vendorPricing[surgeonVendor];
                  const sortedPrices = [...surgeonVendorData.prices].sort((a, b) => a - b);
                  const median = sortedPrices[Math.floor(sortedPrices.length / 2)];
                  vendorStats.push({
                    vendor: surgeonVendor,
                    medianPrice: median,
                    sampleSize: surgeonVendorData.prices.length,
                    totalQuantity: surgeonVendorData.totalQuantity
                  });
                }
                    ...
                    <table className="w-full text-sm border-collapse">
                      <thead>
                        <tr className="bg-gradient-to-r from-purple-100 to-blue-100">
                          <th className="px-3 py-3 text-left font-bold text-gray-900 border-b-2 border-purple-300">#</th>
                          <th className="px-3 py-3 text-left font-bold text-gray-900 border-b-2 border-purple-300">Component Category</th>
                          <th className="px-3 py-3 text-center font-bold text-gray-900 border-b-2 border-purple-300">Your Vendor</th>
                          <th className="px-3 py-3 text-right font-bold text-gray-900 border-b-2 border-purple-300">Your Price</th>
                          <th className="px-3 py-3 text-right font-bold text-gray-900 border-b-2 border-purple-300">Best Price</th>
                          <th className="px-3 py-3 text-right font-bold text-gray-900 border-b-2 border-purple-300">Annual Qty</th>
                          <th className="px-3 py-3 text-right font-bold text-gray-900 border-b-2 border-purple-300">Annual Spend</th>
                          <th className="px-3 py-3 text-right font-bold text-gray-900 border-b-2 border-purple-300">Potential Savings</th>
                        </tr>
                      </thead>
                      <tbody>
                      {getCurrentViewData.topComponents.filter(comp => comp.category !== 'UNKNOWN').slice(0, 5).map((comp, idx) => {
                        ...
                        // Calculate potential savings: compare your vendor's median vs best vendor's median
                        // If you're already using the best vendor, savings = $0
                        const potentialSavings = yourVendorStats && yourVendorStats.vendor !== bestPrice.vendor
                          ? comp.quantity * (yourVendorStats.medianPrice - bestPrice.medianPrice)
                          : 0;

                        const isUsingBestVendor = yourVendorStats && yourVendorStats.vendor === bestPrice.vendor;

                        return (
                          <tr key={idx} className={`border-b border-gray-200 hover:bg-purple-50 ${isUsingBestVendor ? 'bg-green-50' : ''}`}>
                            ...
                            <td className="px-3 py-3 font-semibold text-gray-900">{comp.category}</td>
                            <td className="px-3 py-3 text-center">
                              <div className="font-semibold text-purple-600">{comp.vendor.split(' ')[0]}</div>
                              {isUsingBestVendor && (
                            ...
                            <td className="px-3 py-3 text-right font-semibold text-green-600">
                              ${comp.spend.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                            </td>
                            <td className="px-3 py-3 text-right">
                              {potentialSavings > 0 ? (
                                <div>
                                ...
                                <div className="text-sm font-semibold text-green-600">$0.00</div>
                              )}
                            </td>
                          </tr>
                        );
